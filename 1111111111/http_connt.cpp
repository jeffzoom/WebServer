 ...
 * @Date: 2023-05-12 20:50:24
 * @LastEditors: error: git config user.name & please set dead value or install git
 * @LastEditTime: 2023-05-16 13:51:25
 * @FilePath: /Linux_nc/WebServer_202304/1111111111/http_connt.cpp
 * @Descripttion: 
 */

/*///////////////////////////
2023年05月12日
./webbench -c 7000 -t 5 http://192.168.18.192:10000/indx.html
Requests: 139258 susceed, 0 failed.

这个是一个bug文件
压力测试7000个请求，没有失败的，但是只有24个成功
Requests: 24 susceed, 0 failed.
问题就出在这个文件了
///////////////////////////*/
...
// 网站的根目录  http://192.168.18.187:10000/index.html
const char* doc_root = "/home/nowcoder/Linux_nc/WebServer_202304/myWebServer/resources";
...
void addfd(int epollfd, int fd, bool one_shot) {
    epoll_event event;
    event.data.fd = fd;
    event.events = EPOLLIN | EPOLLET | EPOLLRDHUP; // read()测试，
...
// 如果不重新修改EPOLLONESHOT事件，那么EPOLLIN事件只能被触发一次
...
bool http_conn::read() {
    ...
    while (true) {
        ...
        if (bytes_read == -1) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) { // 非阻塞的去读会有这两个错误，表示没有数据了
                // 没有数据
                break;
            }
            return false;
...
*/
// 主状态机，解析HTTP请求  语法错误：HTTP_CODE http_conn::process_read() {}会报错，需要在HTTP_CODE加上http_conn::
http_conn::HTTP_CODE http_conn::process_read() {
    ...
    while ( ((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK))
        ...
        // 根据/r /n 解析请求
        // 解析到了请求体，完整的数据；或者是解析到了一行完整的数据

        // 获取一行数据
        text = get_line();

        m_start_line = m_checked_idx; // 不懂
        printf("获得一行http信息：%s", text);

        switch (m_check_state) {

            case CHECK_STATE_REQUESTLINE:   // 正在分析请求行（第一行）
            ...
            case CHECK_STATE_HEADER:  // 正在分析请求头（第一行下面的，回车符和换行符上面的）
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text) { // 懂了，要耐心
    
    // 如果用正则会简单，相当的简单，这里用传统方式，优化
    // 字符串切割，用\0，因为\0是字符串结束符
...
// 解析请求头
http_conn::HTTP_CODE http_conn::parse_headers(char* text) { // 懂了
    ...
    if( text[0] == '\0' ) {
        ...
        text += strspn( text, " \t" ); //返回 str1 中第一个不在字符串 str2 中出现的字符下标，，也就是1，指向k
        ...
        printf( "oop! unknow (未知的请求头) header: %s\n", text );
    }
    return NO_REQUEST;
}
...
http_conn::HTTP_CODE http_conn::parse_content(char* text) { // 不懂，重新看
...
http_conn::LINE_STATUS http_conn::parse_line() {
    char temp;
    for (; m_checked_idx < m_read_idx; ++m_checked_idx) { // 不懂为什么是< m_read_idx
...
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    // m_real_file一开始是空的
    strcpy(m_real_file, doc_root); // 把 str2 所指向的字符串复制到 str1 中，m_real_file=/home/nowcoder/webserver/resources
    ...
    // m_real_file=/home/nowcoder/webserver/resources/index.html
...
// 非阻塞的写 一次性写完数据
bool http_conn::write() { // 不懂
    printf("测试write测试\n");
    // m_write_buf写缓冲区中待发送的字节数
    // m_file_address客户请求的目标文件被mmap到内存中的起始位置
    ...
    while(1) {
        ...
        if (bytes_have_send >= m_iv[0].iov_len) { // 已经发送的字节数大于
            m_iv[0].iov_len = 0;
...
// 往写缓冲中写入待发送的数据，数据写到m_write_buf这个数组当中
// 字符串" "的类型就是const char*
bool http_conn::add_response(const char* format, ...) { // const char* format是格式，...是可变参数
    ...
    // vsnprintf()用于向一个字符串缓冲区打印格式化字符串，且可以限定打印的格式化字符串的最大长度 C99或者C++11
...
// 发送回去的内容，一个完整的服务器应该根据不同的资源，比如请求的是MP3文件，Content-Type就不是text/html
...
bool http_conn::process_write(HTTP_CODE ret) { // 不懂
    switch (ret)
    {
        case INTERNAL_ERROR:
        ...
        case FILE_REQUEST:                          // 文件请求完整,获取文件成功
            ...
            m_iv[0].iov_base = m_write_buf;         // 获取第一块数据
            m_iv[0].iov_len = m_write_idx;
            m_iv[1].iov_base = m_file_address;      // 获取第二块数据
            m_iv[1].iov_len = m_file_stat.st_size;
            m_iv_count = 2;                         // 一共两块内存

            bytes_to_send = m_write_idx + m_file_stat.st_size; // 不懂
            return true;
