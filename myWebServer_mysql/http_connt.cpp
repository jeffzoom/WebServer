 ...
 * @LastEditTime: 2023-05-23 09:57:21
...
void addfd(int epollfd, int fd, bool one_shot) {
    epoll_event event;
    event.data.fd = fd;
    // event.events = EPOLLIN | EPOLLET | EPOLLRDHUP; // read()测试，错误，EPOLLET边沿触发没有删掉，这不会就是我那两个bug的原因吧
...
// 为了生成响应报文
bool http_conn::process_write(HTTP_CODE ret) { // 不懂，懂了，其实就是服务器响应
    switch (ret)
    {
